...
import { Mail, CheckCircle, Clock, Inbox, Reply, RefreshCw, CheckCircle2 } from 'lucide-react';
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="bg-card border border-border p-16 text-center rounded-3xl space-y-3">
            ...
            <p className="text-sm text-muted-foreground">No inquiries found matching this view.</p>
          </div>
        ) : (
          filtered.map(ticket => {
            ...
            return (
              <div
                key={ticket.id}
                className="bg-card border border-border hover:border-secondary/30 p-6 rounded-3xl shadow-card transition-all space-y-4"
              >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!isResolved && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl gradient-brand text-white text-xs font-bold shadow-button hover:opacity-90"
                      >
                ...
                <div className="space-y-1">
                  <h3 className="text-lg font-bold text-foreground">{ticket.subject || 'General Inquiry'}</h3>
                ...
                <div className="bg-muted/40 p-4 rounded-2xl border border-border text-foreground/90 whitespace-pre-wrap text-sm leading-relaxed">
                  {ticket.message}
                </div>
              </div>
            );
